...
def generar_caso_de_uso_seleccion_y_validacion():
    ...
    n_samples = int(rng.integers(90, 170))
    n_features = int(rng.integers(10, 25))
    target_col = "target"
    k_features = 3

    # Genera una mezcla de variables informativas y ruido para evitar casos triviales.
    x = rng.normal(0, 1, size=(n_samples, n_features))
    coef = rng.normal(0, 1, size=n_features)

    # Asegura clases balanceadas para CV estratificado.
    for _ in range(100):
        logits = x @ coef + rng.normal(0, 0.5, size=n_samples)
